                ...
                <div className="flex items-center gap-3 w-full md:w-auto pt-4 md:pt-0 border-t md:border-0 border-slate-100">
                  ...
                  {!isSelectMode && (
                    <>
                      ...
                      {/* 3rd Button: Plot Summary */}
                      <button 
                        onClick={(e) => {
                           e.stopPropagation();
                           const count = project.plotting_data?.plotSales?.length || 0;
                           const revenue = project.plotting_data?.plotSales?.reduce((acc: number, p: any) => acc + (Number(p.totalReceived) || 0), 0) || 0;
                           alert(`Sold Plots: ${count}\nTotal Collections: ${formatCurrency(revenue)}`);
                        }} 
                        type="button"
                        className="text-slate-400 hover:text-slate-600 hover:bg-slate-100 px-3 py-2 rounded-lg transition-colors"
                        title="View Plot Summary"
                      >
                        <Users size={18} />
                      </button>
                    </>
